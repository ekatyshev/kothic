...
def get_vectors(minzoom, maxzoom, x, y, style, vec, extent, locales):
    ...
    if "name:en" in column_names_needed:
        ...
        replace(replace(replace(replace(replace(replace(replace(replace(replace(translate(tags->'name:ka','აბგდევზთიკლმნოპრსტუფქყჯჰ','abgdevztiklmnoprstupkqjh'),'ძ','dz'),'წ','ts'),'ჭ','ch'),'შ','sh'),'ჩ','ch'),'ც','ts'),'ღ','gh'),'ხ','kh'),'ჟ','zh'))"""
