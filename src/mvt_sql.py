...
def get_vectors(minzoom, maxzoom, x, y, style, vec, extent, locales):
    ...
    if "name:en" in column_names_needed:
        ...
        replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(translate(tags->'name:ka','აბგდევზთილმნორსუფქჯჰ','abgdevztilmnorsupkjh'),'ძ','dz'),'წ','ts’'),'ჭ','ch’'),'ყ','q’'),'ტ','t’'),'პ','p’'),'შ','sh'),'ჩ','ch'),'ც','ts'),'ღ','gh'),'კ','k’'))"""
